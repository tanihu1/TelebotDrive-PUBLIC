...
def get_help(message):
    ...
    try:
        ...
        if message.chat.type == 'private':
            ...
            activity_log.info(
                f"Sent help message to {message.from_user.first_name + ' ' + message.from_user.last_name} in {message.chat.title}")
...
def credits(message):
    ...
    try:
        if message.chat.type == 'private':
            ...
            bot.reply_to(message, reply, parse_mode='MarkdownV2')
            activity_log.info(
                f"Sent credits to {message.from_user.first_name + ' ' + message.from_user.last_name} in {message.chat.title}")
    except Exception as e:
        logging.exception(
            f"Could not send the reply to {message.from_user.first_name + ' ' + message.from_user.last_name} at {message.from_user.id}")
